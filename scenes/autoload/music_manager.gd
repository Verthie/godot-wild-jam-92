...
func play_music(type: MusicTrack.MusicType, player_num: int = 0, fade_duration: float = 1.0, fade_in: bool = true) -> void:
	...
	if chosen_player.playing:
		_crossfade_to(chosen_player, track, fade_duration)
	else:
		if fade_in:
...
func _crossfade_to(player: AudioStreamPlayer, new_track: MusicTrack, duration: float) -> void:
	# Swap players
	var temp = chosen_player
	chosen_player = fading_player
	fading_player = temp

	# Start new track
	_play_track(chosen_player, new_track)
	chosen_player.volume_db = -80

	# Create fade tween
	var tween = create_tween().set_parallel(true)
	tween.tween_property(chosen_player, "volume_db", new_track.volume, duration)
	tween.tween_property(fading_player, "volume_db", -80, duration)
	tween.chain().tween_callback(fading_player.stop)
